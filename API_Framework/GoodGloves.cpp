...
CGoodGloves::CGoodGloves()
{
}

CGoodGloves::CGoodGloves(float _fX, float _fY, ITEMSTATE::STATE _eState)
{
	m_tInfo.fX = _fX;
	m_tInfo.fY = _fY;
	m_eState = _eState;
...
void CGoodGloves::Initialize()
{
	m_pFrameKey = L"Gloves1";
	m_pBackFrame = L"";
	m_pDropFrame = L"ArmorDrop";
	m_tInfo.iCX = 104;
	m_tInfo.iCY = 76;
	m_eType = ITEMTYPE::GLOVES;
	m_tItemInfo = {};
	m_tItemInfo.iDex = 10;
	m_tItemInfo.iGold = 300;
}

void CGoodGloves::Set_ItemState(ITEMSTATE::STATE _eState)
{
	CSoundMgr::Get_Instance()->StopSound(CSoundMgr::ITEM);
	CSoundMgr::Get_Instance()->PlaySound(L"iglovesmetal.wav", CSoundMgr::ITEM);
...
void CGoodGloves::FieldRender(HDC _DC)
{
